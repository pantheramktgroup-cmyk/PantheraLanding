    ...
    () => {
      ...
      mm.add('(min-width: 768px)', () => {
        ...
        const ctx = gsap.context(() => {
          ...
          // Timeline: panels fill (TOTAL_PANELS-1) units, hold fills 1 unit
          ...
          tl.to(track, { x: () => -lastPanel.offsetLeft, ease: 'none', duration: TOTAL_PANELS - 1 })
  ...
  return (
    <section className="bg-[#080808] overflow-hidden">
      <div className="container-panthera pt-28 md:pt-36 pb-20 md:pb-28">
        <p className="font-sans text-[10px] uppercase tracking-[0.2em] font-semibold text-panthera-white/88 mb-5">{apexSystem.eyebrow}</p>
      ...
      <div ref={pinRef} className="relative hidden md:block" style={{ height: '100vh', overflow: 'hidden' }}>
        <div className="absolute left-0 right-0 z-30 pointer-events-none" style={{ top: 'calc(88px + 2.4rem)' }}>
          <div className="container-panthera flex items-center justify-between">
            <div className="text-panthera-white/82 font-sans text-[11px] uppercase tracking-[0.15em] font-semibold">{apexSystem.eyebrow}</div>
        ...
        <div ref={trackRef} className="flex flex-nowrap h-full bg-panthera-deep" style={{ willChange: 'transform' }}>
          {apexSystem.phases.map((phase, i) => (
            <div key={phase.number} className="relative overflow-hidden bg-panthera-deep h-full" style={{ flex: '0 0 100vw', width: '100vw', minWidth: '100vw', maxWidth: 'none' }}>
              <div className="absolute inset-0" aria-hidden="true">
                <img src={PANEL_IMAGES[i % PANEL_IMAGES.length]} alt="" className="phase-bg w-full h-full object-cover object-center" style={{ filter: METHOD_IMAGE_FILTER }} loading="lazy" />
              ...
              <div className="phase-content absolute z-10 bottom-0 left-0 right-0 container-panthera pb-16 md:pb-20">
                <p className="font-serif text-[18vw] text-panthera-white/[0.12] leading-none select-none -mb-6 pointer-events-none" style={{ textShadow: '0 0 18px rgba(245,245,245,0.08)' }} aria-hidden="true">
                  {phase.number}
                </p>
          ...
          <div className="relative flex items-center justify-center text-center h-full" style={{ flex: '0 0 100vw', width: '100vw', minWidth: '100vw', maxWidth: 'none' }}>
            <img src="/images/creation_panthera_hand.webp" alt="" aria-hidden="true" className="absolute inset-0 w-full h-full object-cover" style={{ filter: METHOD_IMAGE_FILTER }} />
            <div className="absolute inset-0" style={{ background: 'linear-gradient(90deg, rgba(0,0,0,0.68) 0%, rgba(0,0,0,0.48) 45%, rgba(0,0,0,0.38) 100%)' }} aria-hidden="true" />
            <div className="grain-overlay opacity-[0.08]" aria-hidden="true" />
            <div className="relative z-10 w-full max-w-2xl mx-auto px-6" style={{ transform: 'translateY(72px)' }}>
              <p className="font-serif text-panthera-white leading-tight mb-16" style={{ fontSize: 'clamp(1.6rem, 3vw, 3rem)' }}>
      ...
      <div className="md:hidden">
        {apexSystem.phases.map((phase, i) => (
          <div key={`mob-${phase.number}`} className="relative border-t border-[rgba(245,245,245,0.08)] py-16 px-6" style={{ minHeight: '70vh' }}>
            <div className="absolute inset-0" aria-hidden="true">
              <img src={PANEL_IMAGES[i % PANEL_IMAGES.length]} alt="" className="w-full h-full object-cover" style={{ filter: METHOD_IMAGE_FILTER }} loading="lazy" />
