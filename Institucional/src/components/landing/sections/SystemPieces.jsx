  ...
  return (
    <section className="relative bg-black section-pad overflow-hidden">
      ...
      <div ref={containerRef} className="relative z-10 container-panthera">
        {/* Header */}
        <div className="max-w-xl mb-16 md:mb-14">
          <p className="font-sans text-[10px] uppercase tracking-[0.2em] text-panthera-green mb-4">
            {systemPieces.eyebrow}
          </p>

          <h2 className="md:hidden font-serif text-panthera-white leading-tight mb-4" style={{ fontSize: 'clamp(1.8rem, 3.5vw, 3rem)' }}>
            {systemPieces.headline.replace(/\n/g, ' ')}
          </h2>

          <h2 className="hidden md:block font-serif text-panthera-white leading-tight mb-4" style={{ fontSize: 'clamp(1.8rem, 3.5vw, 3rem)' }}>
            {systemPieces.headline}
          </h2>

          <p className="md:hidden font-sans text-sm text-panthera-ash leading-relaxed">
            {systemPieces.subheadline.replace(/\n/g, ' ')}
          </p>

          <p className="hidden md:block font-sans text-sm text-panthera-ash leading-relaxed whitespace-pre-line">
        ...
        <div className="hidden md:grid grid-cols-2 md:grid-cols-4 border-t border-l border-[rgba(245,245,245,0.04)]">
          {systemPieces.pieces.map((piece, i) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-[rgba(0,0,0,0.78)] group-hover:bg-[rgba(0,0,0,0.5)] transition-colors duration-700" />
              ...
              <div
                className="relative z-20 p-6 md:p-8 flex flex-col h-full"
                style={{ minHeight: '240px' }}
              >
                <div className="pt-[58%]">
                  <span className="font-sans text-[10px] text-panthera-green/55 tabular-nums shrink-0 block mb-3">
                    {String(i + 1).padStart(2, '0')}
                  </span>
                  <div className="w-5 h-px bg-panthera-green mb-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />

                  <h3 className="font-serif text-lg md:text-xl text-panthera-white group-hover:text-panthera-cream transition-colors duration-300 mb-2 leading-tight">
                    {piece.title}
                  </h3>
        ...
        <div className="mt-16 md:mt-14 flex justify-center md:justify-start">
